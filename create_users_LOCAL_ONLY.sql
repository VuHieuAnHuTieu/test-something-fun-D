...
CREATE USER IF NOT EXISTS 'bi_reader'@'localhost' 
    IDENTIFIED BY 'bi_reader_secret';
...
CREATE USER IF NOT EXISTS 'agent_app'@'localhost' 
    IDENTIFIED BY 'agent_app_secret';
...
-- Grant runtime DML permissions ONLY on agent_system state tables (DELETE withheld to preserve audit immutability)
